...
<body>
<?php
require_once(MODULES_PATH.'ArtworkCaleSearchPrint.php');
$obj = new ArtworkCaleSearchPrint($_GET);
$result = $obj->getData();
?>
<center>
<div class="TitlePage">
  ...
  <tr>
    <td width="564" height="400" align="center" style="padding:1px ">
	  <table width="560" height="100%" border="0" cellpadding="0" cellspacing="0" >
	    <tr><td class="Thumbnail"><img src="http://picasso.shsu.edu/graphics/<?=$row['StartYear']?>/ythumbs/y<?=$obj->imgName($row['opp'])?>.jpg" title="<?=$row['title']?>"/></td>
		<td class="infoLarge1">
	   <span><strong><font color="#800000"><?=$obj->parseTextTM($AcceptedTitle)?></font></strong></span><br/>
	   <?=$row['location']?>.  
	   <?=$row['duration']?>.<br/> 
	   <span class="MediumDim"><?=$obj->parseTextTM($row['medium'])?>.<br/> 
...
<strong><font color="#800000"><?=$row['opp']?></font></strong><?php  $result1 = $obj->getCatalog($row['opp']); foreach($result1 as $cata) echo("; ".$cata); if($row['bookCatalog']!='') echo("; ".$obj->sortBooks($row['bookCatalog']));?>
	  ...
	  <td class="Thumbnail"><img src="http://picasso.shsu.edu/graphics/<?=$row['StartYear']?>/ythumbs/y<?=$obj->imgName($row['opp'])?>.jpg" title="<?=$row['title']?>"/></td></tr></table>
	  <?php } else echo("&nbsp;");?>
	</td>
  </tr>
</table>
