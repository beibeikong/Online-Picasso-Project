...
<?php $i = 0; while($row = mysql_fetch_array($result)){ ?>
  <tr <?=($i%2==0)? "class=\"Even\"" : ""?>>
       ...
       <td width="85" align="left" valign="middle">
           <a href="index.php?view=ArtworkInfo&OPPID=<?=$row['opp']?>"  class="SmallControl" target="_blank" title="View Artwork Details" onclick="OpenWin(this.href, 'ArtworkInfo', 740, 550); return false;"><img src="./images/tinyicon-text.png" alt=""/>&nbsp;Details</a>
		<?php
		if (isset($_SESSION['UserType'])) {
		if ($_SESSION['UserType'] == 'admin' || $row['notVerified'] == 0)
		{ ?><br><br>
        	<a href="index.php?view=zoom&alpha=<?=$obj->imgName($row['opp'])?>&random=<?=$_GET['startYear']*23?>" onclick="OpenWin(this.href, 'ArtworkZoom', 850, 600); return false;" class="SmallControl" target="_blank" title="Zoom Artwork Image"><img src="./images/tinyicon-magnify.png" alt=""/>&nbsp;Zoom</a>
      		  <?php } }?>
	 	<?php if(trim($row['notes'])!="") { ?> <br><br>
		  <a href="index.php?view=ArtworkNote&OPPID=<?=$row['opp']?>" target="_blank" onclick="OpenWin(this.href, 'ArtworkNote', 740, 550); return false;"  class="SmallControl" title="View Artwork Note"><img src="./images/tinyicon-text.png" alt=""/>&nbsp;Note</a>
		  <?php } ?>
          	<?php if(trim($row['commentary'])!="") { ?> <br><br>
		  <a href="index.php?view=ArtworkCommentary&OPPID=<?=$row['opp']?>" target="_blank" onclick="OpenWin(this.href, 'ArtworkCommentary', 740, 550); return false;"  class="SmallControl" title="View Artwork Commentary"><img src="./images/tinyicon-text.png" alt=""/>&nbsp;Commentary</a>
          	<?php } ?>
      </td>
	...
	?>
     ...
     <td valign="top" height="116" style="padding:4px">
      <span><strong><font color="#800000"><?=$row['title']?></font></strong></span>. <?=$row['location']?>. <?=$row['duration']?>. 
