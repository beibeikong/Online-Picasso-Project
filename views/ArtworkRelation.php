...
<title>Artwork Relations - <?=PROJECTNAME?></title>
...
<?php
require_once(MODULES_PATH.'ArtworkRelation.php');
$obj = new ArtworkRelation();
$OPPMasterInfo = $obj->getInfo($_GET['OPPID']);
$left_row = mysql_fetch_array($OPPMasterInfo);
$RelationOPPList = $obj->getRelations($_GET['OPPID']);
$RelationOPP = (isset($_GET['RelationOPP']))? $_GET['RelationOPP'] : $RelationOPPList[0];  // get RelationOpp
$RelationInfo = $obj->getInfo($RelationOPP);
$right_row = mysql_fetch_array($RelationInfo);
...
<table class="HeaderBar" cellspacing="0">
  <tr>
    ...
    <td class="Title">Artwork Relations</td>
    <td class="Toolbar">
      <table cellspacing="0" align="right">
        <tr>
          <?php if ($_SESSION['UserType'] == 'admin' ){?>
		  <td><a id="NtAndComtry" href="index.php?view=Relationmultiple&OPPID=<? echo $_GET['OPPID'] ?> ">Multiple</a></td>
		  <?php }?>
...
<table id="ArtworkCompare" cellspacing="0" align="center">
 <tr>
    <td id="ImgHolder"><a href="index.php?view=ArtworkInfo&OPPID=<?=$left_row['opp']?>" target="_blank" onclick="OpenWin(this.href, 'ArtworkInfo', 740, 550); return false;">
	...
	<?php if ($_SESSION['UserType'] == 'admin' || $left_row['notVerified'] == 0){?>
	<img src="../graphics/<?=$right_row['startyear']?>/ythumbs/y<?=$obj->imgName($right_row['opp'])?>.jpg" class="imageStyle" title="<?=$left_row['title']?>"/>
	<?php }
	else {?>
	<img src="https://picasso.shsu.edu/images/yopp-emblem-innershadow.jpg"/>
	<?php }?>
  ...
  <tr class="ArtworksInfo">
	<td style="padding-top:5px "><table width="300" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <?php }?>
      <tr>
        <td class="InfoLabel">Catalog:</td> <td class="Info"><strong><font color="#800000"><?=$left_row['opp']?></font></strong><?php  $result = $obj->getCatalog($left_row['opp']); foreach($result as $cata) echo("; ".$cata); ?>
    ...
    <td colspan="2" id="AltsHolder" align="center">
	  <center><div>
        <table border="0" cellspacing="5" cellpadding="2" align="center">
		  <tr>
		  <?php
		  foreach($RelationOPPList as $relation) { ?>
			<td class="singleImage"><?php echo $obj->getOneOPP($relation, $_SERVER['QUERY_STRING'], $_GET);?></td>
		  <?php } ?>
		  </tr>
		</table>
	  </div></center>
