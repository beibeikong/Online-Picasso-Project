          ...
          <td>
            <table class="TabContainer" id="MenuTabs" cellspacing="0" align="center">
              <tr class="Tabs">
				...
				<td class="Inactive"><a href="index.php?<?=str_replace("BioCalendarSearch","ArtworkCaleSearchS",$_SERVER['QUERY_STRING'])?>"><img src="./images/magnify-text.png"/>Artwork Summary</a></td>				
				<td class="Inactive"><a href="index.php?<?=str_replace("BioCalendarSearch","ArtworkCaleSearchD",$_SERVER['QUERY_STRING'])?>"><img src="./images/magnify-text.png"/>Artwork Display</a></td>
  ...
  <tr>
	...
	<td width="150px" align="right" >
	  <table cellspacing="0" style="clear:none; float:right">
	    <tr>
          <td>
            <a class="PageButton" href="index.php?<?=str_replace("BioCalendarSearch", "printBioSearch", $_SERVER['QUERY_STRING'])?>" target="_blank" onclick="if(!PrintWarning()) return false;" ><img src="./images/icon-print.gif" width="16" height="16" border="0"/><br/>Print</a>
		  </td>
		</tr>
      </table>
	</td>
...
?>
  <tr>
    <td class="duration"><?=$row['dateDesc']?>:
	 ...
	 <?php if($row['dateDesc']!="Year-Forth") {?>
			 <?php if($row['DatedArtworks']>0 && $row['DatedArtworks']<=5) { ?>
			   <a  href="index.php?view=DatedArtworks&startYear=<?=$row['StartYear']?>&startMonth=<?=$row['StartMonth']?>&startDay=<?=$row['StartDay']?>&endYear=<?=$row['EndYear']?>&endMonth=<?=$row['EndMonth']?>&endDay=<?=$row['EndDay']?>" class="Biotab" target="_blank" title="View Artworks made during this timeframe"  onclick="OpenWin(this.href, 'artwork', 660, 320); return false;">Artworks</a>
			 <?php } ?>
			 
			 <?php if($row['DatedArtworks']>5) { ?>
			   <a  href="index.php?view=DatedArtworks&startYear=<?=$row['StartYear']?>&startMonth=<?=$row['StartMonth']?>&startDay=<?=$row['StartDay']?>&endYear=<?=$row['EndYear']?>&endMonth=<?=$row['EndMonth']?>&endDay=<?=$row['EndDay']?>" class="Biotab" target="_blank" title="View Artworks made during this timeframe"  onclick="OpenWin(this.href, 'artwork', 660, 550); return false;">Artworks</a>
			 <?php } ?>
	 <?php } ?>		
	 ...
	 ?>
	   <a href="index.php?view=BioWritings&mid=<?=$writingsRow['mid']?>&year=<?=$row['StartYear']?><?=str_replace("view=BioCalendarSearch","",$_SERVER['QUERY_STRING'])?>" class="Biotab" target="_blank" title="Open commentary in a pop-up window" onclick="OpenWin(this.href, 'Writings', 748, 800); return false;">Writings</a>
     <?php } } ?>
	...
	?>
		...
		<a  href="index.php?view=BioPhoto&type=photo&year=<?=$currentYear?>&img=<?=$obj->parsePhotoName($photoName)?>" target="_blank" onclick="OpenWin(this.href, 'BioPhoto', 850, 650);  return false;" class="Biotab" title="<?=$photoDescrption?>">Photo</a>   
