...
<?php $i = 1; while($row = mysql_fetch_array($result)){ ?>  
  <tr <?=($i%2!=0)? "class=\"Even\"" : ""?>>
    ...
    <td class="Catalogs">
	  ...
	  <strong><font color="#800000">[<?=$row['opp']?>]</font></strong>
	  <?php } else { ?><strong><font color="#800000"><?=$row['opp']?></font></strong><?php }  $result1 = $obj->getCatalog($row['opp']); foreach($result1 as $cata) echo("<br>".$cata); ?>
	</td>
    ...
    <td class="Details">
	   <span><strong><font color="#800000"><?php $temp=trim($row['title']); $temp=str_replace("&","&amp;",$temp);$temp=str_replace("<","&lt;",$temp);$temp=str_replace(">","&gt;",$temp); echo $temp;?>.</font></strong></span><br><?=$row['location']?>.&nbsp;<?=$row['duration']?>. 
      <span class="MediumDim"><?php $temp=trim($row['medium']); $temp=str_replace("&","&amp;",$temp);$temp=str_replace("<","&lt;",$temp);$temp=str_replace(">","&gt;",$temp); echo $temp;?>.&nbsp;<?=$row['dimension']?>.&nbsp;</span><?=$row['collection']?>. 
	...
	<td class="Checks"><?php $result1 = $obj->getFormer($row['opp']); if(mysql_num_rows($result1)>0) echo "<span class=\"true\">✔</span>"; else echo "<span class=\"false\">✘</span>"; ?></td>
	<td class="Checks"><?php $result2 = $obj->getOPP($row['opp']); if(count($result2)!=0) echo "<span class=\"true\">✔</span>"; else echo "<span class=\"false\">✘</span>"; ?></td>
