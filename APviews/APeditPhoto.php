...
<title>Edit Photo - <?=PROJECTNAME?></title>
...
<body>
<?php
require_once(APMODULES_PATH.'ModifyPhoto.php');
$obj = new ModifyPhoto($_GET['id'],$_GET['no']);
$row = $obj->getData();
$daterow = $obj->getBioDate();
$image = $row['name'];
...
<input value="<?=$_GET['no']?>" id="no" name="no" type="hidden">
		  ...
		  <div id="Tab1Side">
			<table class="DataEntry" cellspacing="2">
			  <tr>	
				...
				<td><input value="<?=$daterow['dateDesc']?>" size="30" id="desc" name="desc" type="text"></td>
			  </tr>
              <tr>	
			    <td class="Label"><label for="title">Year:</label> </td>
				<td><input value="<?=$daterow['StartYear']?>" size="30" id="year" name="year" type="text"></td>
			  </tr>
              <tr>	
              ...
              <tr>	
                ...
                <td>
				<textarea name="description" id="description" rows="10" cols="30"><?=$row['description']?></textarea>
               </td>
			  </tr>
              </table>
</div>
...
<table class="ActionButtons" cellspacing="0" align="center">
  <tr>
    ...
    <div>
	  <table class="StatusHolder" cellspacing="0">
		<tr>
		  ...
		  <td>
			<button type="button" onclick="OnSaveModifiedPhotoForm('ModifyPhotoEngine'); return false;" id="SaveButton">Save</button>
		  </td>
	...
	<td>
	<div>
	  <button type="button" onclick="PreviewEntry(this.form, 'Photo','Photo', 740, 650)">Preview</button>
