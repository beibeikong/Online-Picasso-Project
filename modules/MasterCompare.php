...
class MasterCompare extends Database
{
...
public function getMasterInfo($MasterID)  //operate on database to get data
  {
...
public function getOpps($MasterID)  //operate on database to get data
  {
	$opp = mysql_real_escape_string($MasterID);
	$query = "SELECT opp FROM `RLTN_MASTERARTWORK_ARTWORK` WHERE masteropp = '$opp'";
	$result = mysql_query($query);
	$result_array=array();
...
public function getOneOPP($oppID, $href, $param)
  {
   ...
   $query = "SELECT opp,title, YEAR(dateStart) as year FROM `ARTWORK` WHERE opp = '$opp'";
	...
	if(isset($param['OPP']))
{  
	  $href = str_replace("OPP=".$param['OPP'], "OPP=".$opp, $href);
	}
	else
{
	  $href = $href."&OPP=".$opp;
