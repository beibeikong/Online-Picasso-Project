...
class ArtworkRelation extends Database
{
  ...
  public function getInfo($oppID)  //operate on database to get data
  {
  ...
  public function getRelations($oppID)  //operate on database to get data
  {
	$opp = mysql_real_escape_string($oppID);
	$query = "SELECT opp FROM `RLTN_ARTWORK_ARTWORK` WHERE oppmaster = '$opp'";
	$result = mysql_query($query);
	$result_array=array();
	while($rows=mysql_fetch_assoc($result)){
	//print_r($rows);
		array_push($result_array,$rows['opp']);
	}
  ...
  public function getOneOPP($oppID, $href, $param)
  {
   $opp = mysql_real_escape_string($oppID);
   $query = "SELECT opp,title, YEAR(dateStart) as year FROM `ARTWORK` WHERE opp = '$opp'";
    $result = mysql_query($query);
	$row = mysql_fetch_array($result);
	$href = "index.php?".$href;
	//$href = str_replace(" ", "+", $href);
	if(isset($param['RelationOPP']))
{  
	  $href = str_replace("RelationOPP=".$param['RelationOPP'], "RelationOPP=".$opp, $href);
	}
	else
{
	  $href = $href."&RelationOPP=".$opp;
	}
