...
class Calendar extends Database
{
     ...
     function Calendar($y)
     {
	   parent::__construct(); 
	   $this->year = $y;
	   $this->query = "select count(*) from `NARRATIVE` A where " ;
	   $this->yr = $y . "-";
	 ...
	 function checkMonth($m)
	 {
	   ...
	   if($row[0] >0)
	   {
$l = "index.php?view=BioCalendarSearch&page=1&Keywords=&Commentary=&SearchStyle=Continuous&SearchBy=month&StartMonth=$m&StartYear=$this->year&EndMonth=$m&EndYear=$this->year&Sort=asc";
     ...
     function getMonthHTML($month)
     {
      ...
      while ($d <= $daysInMonth)
      {
          ...
          for ($i = 0; $i < 7; $i++)
          {
              ...
              {
                  $flag = $this->verifyThisDay($month, $d); // verify if there are biographys on $month/$d/$year
				  if($flag==true) // exist
				  {
				    $l = "index.php?view=BioCalendarSearch&page=1&Keywords=&Commentary=&SearchStyle=Continuous&SearchBy=monthday&StartMonth=$month&StartDay=$d&StartYear=$this->year&EndMonth=$month&EndDay=$d&EndYear=$this->year&Sort=asc";
	 ...
	 {
	   if($i==4) {$startYr=$this->year; $endYr=$this->year+1;} else {$startYr=$this->year; $endYr=$this->year;}
	   switch($i)
	   {
	     case 0:
		   $start = $startYr . "-01-01";
	   ...
	   if($row[0] >0)
	   {
	     $l = "index.php?view=BioCalendarSearch&page=1&Keywords=&Commentary=&SearchStyle=Continuous&SearchBy=season&StartSeason=$i&StartYear=$startYr&EndSeason=$i&EndYear=$startYr&Sort=asc";
