...
// =======================================================================================================
      if(strlen($sqlKeywordConstraint) >0)
      {$sqlExhibitConstraint = " and Exhibitions > '' ";}
      else
      {$sqlExhibitConstraint = " where Exhibitions > '' ";}
      
	$sqlOrderBy = $this->getOrderby();
	$query = "SELECT SQL_CALC_FOUND_ROWS id,Author,Books,Catalogs,Title,Date,Publisher,Journal,Edition,Issue,Volume,Exhibitions FROM `REFERENCES` " .$sqlKeywordConstraint.$sqlTimeConstraint.$sqlExhibitConstraint.$sqlOrderBy. " limit $limitStart, $limitEnd";
